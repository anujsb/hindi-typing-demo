...
import { addVideo, updateVideo } from "@/actions/admin"
import { useRouter } from "next/navigation"
import { videoTutorials } from "@/lib/schema"

export default function VideoForm({ initialData }: { initialData?: typeof videoTutorials.$inferSelect }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.error) {
      ...
      if (!initialData) {
        ...
        router.push("/admin/learning-module")
      }
      router.refresh()
    }
    setLoading(false)
  ...
  return (
    <form onSubmit={onSubmit} className="bg-[#262015] p-6 rounded-2xl border border-[#332b1e] space-y-6 font-sans text-sm mt-6">
      <h3 className="text-xl font-bold text-[#faf7f2] mb-4">
        {initialData ? "Edit Learning Day" : "Add Learning Day (Video + Practice Test)"}
      </h3>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <input required type="number" name="day" defaultValue={initialData?.day} placeholder="1" className="w-full bg-[#1c1810] border-[#332b1e] rounded px-3 py-2 text-white" />
        </div>
        <div className="space-y-1">
          <label className="text-[#a0896a]">Display Title</label>
          <input required name="title" defaultValue={initialData?.title} placeholder="Home-Row Video Tutorial" className="w-full bg-[#1c1810] border-[#332b1e] rounded px-3 py-2 text-white" />
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        <input required name="videoUrl" defaultValue={initialData?.videoUrl} placeholder="https://youtube.com/watch?v=..." className="w-full bg-[#1c1810] border-[#332b1e] rounded px-3 py-2 text-white" />
      </div>

      <div className="space-y-1">
        <label className="text-[#a0896a]">Description / Lesson Focus (Optional)</label>
        <textarea name="description" defaultValue={initialData?.description || ""} placeholder="Today we focus on the home row..." className="w-full bg-[#1c1810] border-[#332b1e] rounded px-3 py-2 text-white h-20" />
      </div>

      <div className="space-y-1">
        <label className="text-[#a0896a]">Practice Test Content (For post-video typing test)</label>
        <textarea required name="practiceTestContent" defaultValue={initialData?.practiceTestContent || ""} placeholder="Paste the exact Hindi passage they should type after the video..." className="w-full bg-[#1c1810] border-[#332b1e] rounded px-3 py-2 text-white h-32 font-serif" />
      </div>
      ...
      <button disabled={loading} type="submit" className="w-full mt-4 py-3 bg-[#c9a96e] text-white rounded font-bold hover:bg-[#b5955a] transition-colors">
        {loading ? "Saving..." : (initialData ? "Update Learning Day" : "Save Learning Day")}
      </button>
    </form>
  )
}
